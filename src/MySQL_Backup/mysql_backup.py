  ...
  def restore(self):
    ...
    if self.config.get('misc','backuptype') == "mysqldump":
      ...
      if rest_type == "full":
        ...
        MySQL_Binlog = "mysqlbinlog --no-defaults --stop-datetime=\"" + pitr + "\" "
        ...
        for file in rest_files:
          MySQL_Binlog = "mysqlbinlog --no-defaults --database=" + dbname + " --start-datetime=\"" + starttimes[file] + "\" --stop-datetime=\"" + pitr + "\" "
    ...
    elif self.config.get('misc','backuptype') == "rsync":
      pass
      ...
      if rest_type == "full":
        ...
        if snapshot.create_lvm_snapshot() == 0:
          ...
          if snapshot.mountsnap() == 0:
            ...
            shutil.copy2(str(self.config.get('mycnf','log_error')),str(self.config.get('mycnf','log_error')+"_restore"))
            os.system("> " + str(self.config.get('mycnf','log_error')))
            code, out, err = borg.borgRestore(config.get('fs','borgdir'),bckp_name,"")
            res = out.decode("utf-8")
            res = res.split()
            print(format(res))
        ...
        if snapshot.create_lvm_snapshot() == 0:
          ...
          if snapshot.mountsnap() == 0:
            ...
            self.logger.info("Starting restore of database" + dbname)
            print("Starting restore of database " + dbname)
            code, out, err = borg.borgRestore(config.get('fs','borgdir'),bckp_name,rest_path)
            res = out.decode("utf-8")
            res = res.split()
            print(format(res))
            self.logger(format(res))
